...
if utils.executable("vtsls") then
  lspconfig.vtsls.setup {
    on_attach = function(client, bufnr)
      -- Call your custom attach function
      custom_attach(client, bufnr)

      -- LSP command for move refactoring
    ...
    settings = {
      ...
      javascript = vim.tbl_deep_extend("force", {}, {
        updateImportsOnFileMove = { enabled = "always" },
        suggest = {
          completeFunctionCalls = true,
        },
      ...
      }, {}), -- Ensure settings are copied over from typescript to javascript
    },
    flags = { debounce_text_changes = 500 },
  }

  -- Key mappings specific to vtsls
  ...
  keymap.set("n", "<leader>co", vim.lsp.buf.code_action, { desc = "Organize Imports" })
  keymap.set("n", "<leader>cM", function()
    vim.lsp.buf.execute_command({
      command = "typescript.source.addMissingImports.ts",
    })
  end, { desc = "Add Missing Imports" })
  keymap.set("n", "<leader>cu", function()
    vim.lsp.buf.execute_command({
      command = "typescript.source.removeUnused.ts",
    })
  end, { desc = "Remove Unused Imports" })
  keymap.set("n", "<leader>cD", function()
    vim.lsp.buf.execute_command({
      command = "typescript.source.fixAll.ts",
    })
  ...
  vim.notify("vtsls not found!", vim.log.levels.WARN, { title = "Nvim-config" })
end
...
--       typescript = {
--         preferences = {
--           importModuleSpecifier = "relative",  -- Customize this to your needs
--         },
--       },
--       javascript = {
--         preferences = {
